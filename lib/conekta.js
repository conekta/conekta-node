    ...
    return _.extend({
        ...
        createMembers: function() {
            for (let key in this.members) {
                ...
                for (let attr in this._json) {
                    if (attr == key) {
                        if (this._json[attr] instanceof Array) {
                            let array = [],
                                attribute = this._json[attr];
                            ...
                            if (this._json[attr]) {
                                this[attr] = _.extend(Conekta[member], {
                                    _json: this._json[attr],
                                    _id: this._json[attr].id
                                });
                            }
                        }
                    }
    ...
    createSubscription: function(data, next) {
        this.custom('post', [this.classUrl, this._id, 'subscription'].join('/'), {
            ...
            next: function(err, res) {
                ...
                this.createMembers();
                let subscription = _.extend(Subscription, {
                    _json: res,
                    _id: res.id
                });
    ...
    createPayoutMethod: function(data, next, error) {
        this.custom('post', [this.classUrl, this._id, 'payout_methods'].join('/'), {
            ...
            next: function(err, res) {
                ...
                this.createMembers();
                let payoutMethod = _.extend(PayoutMethod, {
                    _json: res,
                    _id: res.id
                });
