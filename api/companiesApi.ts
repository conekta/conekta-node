...
export class CompaniesApi {
     ...
     */
    public async getCompanies (acceptLanguage?: 'es' | 'en', limit?: number, search?: string, next?: string, previous?: string, options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: GetCompaniesResponse;  }> {
        ...
        return interceptorPromise.then(() => {
            ...
            return new Promise<{ response: http.IncomingMessage; body: GetCompaniesResponse;  }>((resolve, reject) => {
                localVarRequest(localVarRequestOptions, (error, response, body) => {
                    if (error) {
                        reject(error);
                    } else {
                        if (response.statusCode && response.statusCode >= 200 && response.statusCode <= 299) {
                            body = ObjectSerializer.deserialize(body, "GetCompaniesResponse");
     ...
     * @param acceptLanguage Use for knowing which language to use
     */
    public async getCompany (id: string, acceptLanguage?: 'es' | 'en', options: {headers: {[name: string]: string}} = {headers: {}}) : Promise<{ response: http.IncomingMessage; body: CompanyResponse;  }> {
